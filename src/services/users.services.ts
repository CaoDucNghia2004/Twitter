...
class UsersService {
  ...
  async refreshToken({
    ...
    const [new_access_token, new_refresh_token] = await Promise.all([
      ...
      databaseService.refreshTokens.deleteOne({ token: refresh_token }) // ko nên truyền user_id vì có thể 1 user đăng nhập nhiều nơi
    ])
  ...
  async register(payload: RegisterReqBody) {
    // Truyền user_id vào, nhưng mà mới tạo tài khoản thì ko có user_id => tạo user_id bằng code chứ ko tạo bằng mongoDB
    ...
    await databaseService.users.insertOne(
      new User({
        ...payload,
        _id: user_id,
        username: `user${user_id.toString()}`, // Tạo username tạm thời mặc định khi mới tạo tài khoản
        email_verify_token,
  ...
  async verifyEmail(user_id: string) {
    ...
    // Chu kì MongoDB cập nhật giá trị
